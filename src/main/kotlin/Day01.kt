fun main() {
    ...
    fun extractFirstLastNumber(l: String): Int {
        val digits = l.replace(Regex("\\D"), "")
        val firstLast = digits.first().toString() + digits.last().toString()
...
//
//    for (l in allLines) {
//        answer += extractFirstLastNumber(l)
//    }
//
//    println("Part 1: $answer")

    val allNumbers = arrayListOf("zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine")

    val processedLines = allLines.map {
        var newLine = it
        allNumbers.forEachIndexed { i, numName ->
            newLine = newLine.replace(numName, "$numName${i}$numName")
        }
        newLine
    }
    ...
    for (l in processedLines) {
        answer2 += extractFirstLastNumber(l)
    }
