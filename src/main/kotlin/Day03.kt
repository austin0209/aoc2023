fun main() {
    ...
    val cols = schematic.first().length

    fun indexInput(row: Int, col: Int): Char? {
        if (row < 0 || row >= rows || col < 0 || col >= cols) return null
        return schematic[row][col]
    }

    fun isSymbolAdjacentPos(row: Int, col: Int): Boolean {
        val isSymbolRegex = Regex("[^\\d.]")
        val surroundings = arrayOf(
            indexInput(row - 1, col - 1),
            indexInput(row - 1, col),
            indexInput(row - 1, col + 1),
            indexInput(row, col - 1),
            indexInput(row, col + 1),
            indexInput(row + 1, col - 1),
            indexInput(row + 1, col),
            indexInput(row + 1, col + 1),
        ...
        return surroundings.any {
            it != null && isSymbolRegex.matches(it.toString())
        }
    }
    ...
    fun extractPartNumber(row: Int, col: Int): Int? {
        ...
        var currCol = col
        var element = indexInput(row, col)
        var anySymbolAdjacent = false

        while (element != null && numberRegex.matches(element.toString())) {
            anySymbolAdjacent = anySymbolAdjacent || isSymbolAdjacentPos(row, currCol)
            result += element
            currCol += 1
            element = indexInput(row, currCol)
        }

        if (!anySymbolAdjacent || result == "") {
            return null
        }
    ...
    for (r in 0..<rows) {
        var c = 0;
        while (c < cols) {
            val number = extractPartNumber(r, c)
            if (number != null) {
                answer += number
...
private const val INPUT = """..172..............................454..46.......507..........809......923.778..................793..............137.............238........
............*.........712........=.......*................515.*...........*.......690.........../..........658.........=.........*..........
